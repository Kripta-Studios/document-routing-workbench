...
def main():
    ...
    serve=sub.add_parser("serve");serve.add_argument("--host",default="127.0.0.1");serve.add_argument("--port",type=int,default=8770)
    ...
    evaluation=sub.add_parser("evaluate");evaluation.add_argument("--config",default=str(ROOT/"configs/evaluation.json"));evaluation.add_argument("--dry-run",action="store_true")
    ...
    if args.command=="doctor":emit(doctor())
        ...
        emit(evaluate(args.config,args.dry_run))
